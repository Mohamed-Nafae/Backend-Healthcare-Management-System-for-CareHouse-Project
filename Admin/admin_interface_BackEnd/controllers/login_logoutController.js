...
const handleLogin = async (req, res) => {
  ...
  try {
    const response = await axios.get(
      `http://127.0.0.1:5010/api/admins/query?email=${email}`
    );
    const foundUser = response.data;
    // evaluate password
    const match = await bcrypt.compare(password, foundUser.password);
    if (match) {
      const role = foundUser.role_admin;
      ...
      try {
        await axios.put(
          `http://127.0.0.1:5010/api/admins/${foundUser._id}`,
          {
...
const handleLogout = (req, res) => {
  ...
  axios
    .get(`http://127.0.0.1:5010/api/admins/query?refreshToken=${refreshToken}`)
    .then(async (response) => {
      const foundUser = response.data;
